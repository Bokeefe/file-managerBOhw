...
var word2find = "hello";
var word2replace = "goodbye";

fs.readFile("./hello.txt", function(err, data) {//reading a file that is accessable to the server
	if (err) {
		console.log(err);
		return;
	}
	var wordString = data.toString();
	var beatled = "";
	function findWerd(text,word2find,word2replace) {
		beatled = text.replace(word2find,word2replace);
		beatled = beatled.split(word2find).join(word2replace);
		console.log(beatled);
		}

	findWerd(wordString,word2find,word2replace);

	fs.writeFile("hipipsem.txt",beatled,function(err) {//creating a file from the server
		if(err){
			return console.log(err);
		}
		console.log("Replaced " + word2find + " with " + word2replace);
	});
});
